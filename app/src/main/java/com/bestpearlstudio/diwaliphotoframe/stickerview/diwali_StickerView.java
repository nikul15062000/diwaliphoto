...
public abstract class diwali_StickerView extends FrameLayout implements View.OnClickListener {
    ...
    public diwali_StickerView(Context context) {
        super(context);
        init(context);
    }

    public diwali_StickerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public diwali_StickerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if (diwali_StickerView.this.getParent() != null) {
                    ViewGroup myCanvas = ((ViewGroup) diwali_StickerView.this.getParent());
                    myCanvas.removeView(diwali_StickerView.this);
                }
            }
        });
            ...
            if (view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        Log.v(TAG, "sticker1 view action down");
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                    case MotionEvent.ACTION_MOVE:
                        Log.v(TAG, "sticker1 view action move");
                        float offsetX = event.getRawX() - move_orgX;
                        float offsetY = event.getRawY() - move_orgY;
                        diwali_StickerView.this.setX(diwali_StickerView.this.getX() + offsetX);
                        diwali_StickerView.this.setY(diwali_StickerView.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                    case MotionEvent.ACTION_UP:
                        Log.v(TAG, "sticker1 view action up");
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = diwali_StickerView.this.getX();
                        this_orgY = diwali_StickerView.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = diwali_StickerView.this.getLayoutParams().width;
                        scale_orgHeight = diwali_StickerView.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = diwali_StickerView.this.getX() +
                                ((View) diwali_StickerView.this.getParent()).getX() +
                                (float) diwali_StickerView.this.getWidth() / 2;
                        ...
                        centerY = diwali_StickerView.this.getY() +
                                ((View) diwali_StickerView.this.getParent()).getY() +
                                statusBarHeight +
                                (float) diwali_StickerView.this.getHeight() / 2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (length2 > length1
                            ...
                            diwali_StickerView.this.getLayoutParams().width += offset;
                            diwali_StickerView.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        } else if (length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff - 180) < 25)
                                && diwali_StickerView.this.getLayoutParams().width > size / 2
                                && diwali_StickerView.this.getLayoutParams().height > size / 2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
